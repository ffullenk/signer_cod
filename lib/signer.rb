...
class Signer
  ...
  def binary_security_token_node
    ...
    unless node
      ...
      wsu_ns = namespace_prefix(node, WSU_NAMESPACE, "")
  ...
  #   </X509Data>
  # </KeyInfo>
  def x509_data_node(issuer_certificate_in_security_token = true)
    ...
    data_node.add_child(issuer_serial_node) if issuer_certificate_in_security_token
  ...
  def sign!(options = {})
    ...
    if options[:issuer_serial]
      x509_data_node(options[:issuer_certificate])
    end
